...
void insertNode2(HT* table, int location,char* key,void* val){
    KVpair* newN = (KVpair*)malloc(sizeof(KVpair));
    newN->key =key;
    newN->val = val;
    newN->next = table->table[location];
    table->table[location] = newN;
    table->count++;
    //return 0;
}
    ...
    if (LLNode==NULL){
        ...
        while (strcmp(LLNode->key,key)!= 0){
            ...
            if (LLNode==NULL){
                ...
                newN->key =key;
                newN->val = val;
                newN->next = table->table[location];   //the next node would be the next bucket in the LL
                table->table[location] = newN;         // the LL location would be the new node
                table->count++;
                return 0;
            }
        }
